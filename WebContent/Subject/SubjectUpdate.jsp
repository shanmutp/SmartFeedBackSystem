<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1" import="com.smartfeedback.vo.*,java.util.Map,java.util.Set"%>
<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<!--[if lt IE 9]>
	...
	<![endif]-->
<%
	SearchSubjectVO vo = (SearchSubjectVO) request.getAttribute("subjectVO");
	SubjectVO vo1 = (SubjectVO) request.getAttribute("vo");
%>
...
<body
	...
	setSelected("subjectNameId","<%=vo.getSubjectName()%>");
	setSelected("specializationId",<%=vo.getSpecializationId()%>);'>
	<%
		} else if (vo1 != null) {
	%>

<body
	onLoad='setSelected("courseID",<%=vo1.getCourseId()%>);
	setSelected("subjectNameId","<%=vo1.getSubjectName()%>");
	setSelected("specializationId",<%=vo1.getSpecializationId()%>);'>
	<%
		}
	%>
				...
				<div class="row">
					...
					<div class="col-md-6 animate-box">
						<form action="EditSubject" method="post">
							...
							<input type="hidden" value=<%=vo.getSubjectId()%>
								name="subjectId" /> <input type="hidden"
								value=<%=vo.getCourseId()%> name="courseIdHidden" />
							<fieldset>
								<%
									} else if (vo1 != null) {
								%>
								<p class="bg-success">Subject sucessfully updated</p>
								<fieldset disabled>
									<%
										}
									%>
									...
									<div class="row form-group">
										<div class="col-md-12">
											<!-- <label for="message">Message</label> -->
											<input type="text" name="subjectName" id="subjectNameId"
